...
StateLabel PktA,PktB,GemA,GemB,SetA,SetB;//Uchwyty do liczników punktów używane w nadpisanych akcjach przycisków punktowych
...
void draw() //Żeby działały zdarzenia myszy to musi być chociaż puste draw()!
{
  ...
  text("  TENIS: Prerelase version for Iwona PILCHOWSKA <ipilchowska@gmail.com>",0,0,width,10*Ysc); 
}
...
void setup()
{
  ...
  columnNames.add("CourtT");  
  columnNames.add("CourtB"); 
  columnNames.add("Serves"); 
  ...
  tmp=new WrUniqTextButton(serves,"servis",10*Xsc,160*Ysc,50*Xsc,180*Ysc,"srv",3); buttons.add(tmp);serves.add(tmp);
  ...
  //Końcowe
  tmp=new TextButton("TV placeholder",10*Xsc,10*Ysc,230*Xsc,140*Ysc); tmp.back=color(210,210,255);buttons.add(tmp); //na razie nieistotne
