...
void setup()
{
  ...
  //BAZOWY EKRAN MA   static int iniWidth=360;
  //  360x330         static int iniHeight=330;
  //Musi być zaraz za kolumnami zbiorczymi, więc pierwsza na liście przycisków
  others = new Panel(220*Xsc,10*Ysc,350*Xsc,180*Ysc); others.back=color(100,100,100); viAreas.add(others);
  TextButton tmp=new TextButton("Net",30*Xsc,150*Ysc,130*Xsc,160*Ysc);buttons.add(tmp);others.add(tmp);tmp.strokW=1;tmp.corner=0; //courtT.add(tmp); 
  //wymiany
  tmp=Exch=new StateLabel(0,"exchange",10*Xsc,150*Ysc,25*Xsc,170*Ysc); buttons.add(tmp);others.add(tmp);
  
  //Blok logiki zliczania punktów 
  points = new Panel(110*Xsc,230*Ysc,240*Xsc,310*Ysc); viAreas.add(points);
  tmp=PktA=new StateLabel(0,"PktA",43*Xsc,0*Ysc,63*Xsc,20*Ysc); buttons.add(tmp); points.add(tmp);
  tmp=PktB=new StateLabel(0,"PktB",67*Xsc,0*Ysc,87*Xsc,20*Ysc); buttons.add(tmp); points.add(tmp);
  tmp=GemA=new StateLabel(0,"GemA",43*Xsc,30*Ysc,63*Xsc,50*Ysc); buttons.add(tmp); points.add(tmp);
  tmp=GemB=new StateLabel(0,"GemB",67*Xsc,30*Ysc,87*Xsc,50*Ysc); buttons.add(tmp); points.add(tmp);
  tmp=SetA=new StateLabel(0,"SemA",43*Xsc,60*Ysc,63*Xsc,80*Ysc); buttons.add(tmp); points.add(tmp);
  tmp=SetB=new StateLabel(0,"SemB",67*Xsc,60*Ysc,87*Xsc,80*Ysc); buttons.add(tmp); points.add(tmp);
  

  StateLabelInc  incPktA,incPktB,incGemA,incGemB,incSetA,incSetB;//Uchwyty do inkrementatorów
  tmp=incPktA=new StateLabelInc("pktA",0*Xsc,0*Ysc,40*Xsc,20*Ysc,PktA,null); buttons.add(tmp); points.add(tmp);
  tmp=incPktB=new StateLabelInc("pktB",90*Xsc,0*Ysc,130*Xsc,20*Ysc,PktB,null); buttons.add(tmp); points.add(tmp);
  tmp=incGemA=new StateLabelInc("gemA",0*Xsc,30*Ysc,40*Xsc,50*Ysc,GemA,null){ public void flip_state(boolean visual){ //Nakładka metody 
                  super.flip_state(visual);PktA.allow();PktB.allow();
                  }}; buttons.add(tmp); points.add(tmp);
  tmp=incGemB=new StateLabelInc("gemB",90*Xsc,30*Ysc,130*Xsc,50*Ysc,GemB,null){ public void flip_state(boolean visual){ //Nakładka metody 
                  super.flip_state(visual);PktA.allow();PktB.allow();
                  }}; buttons.add(tmp);  points.add(tmp);
  tmp=incSetA=new StateLabelInc("setA",0*Xsc,60*Ysc,40*Xsc,80*Ysc,SetA,null){ public void flip_state(boolean visual){ //Nakładka metody 
                  super.flip_state(visual);PktA.allow();PktB.allow();GemA.allow();GemB.allow();
                  }}; buttons.add(tmp); points.add(tmp);
  tmp=incSetB=new StateLabelInc("setB",90*Xsc,60*Ysc,130*Xsc,80*Ysc,SetB,null){ public void flip_state(boolean visual){ //Nakładka metody 
                  super.flip_state(visual);PktA.allow();PktB.allow();GemA.allow();GemB.allow();
                  }}; buttons.add(tmp); points.add(tmp);   
  ...
  players = new Panel(130*Xsc,160*Ysc,220*Xsc,210*Ysc); viAreas.add(players);
  ...
  courtT = new Panel(250*Xsc,10*Ysc,350*Xsc,160*Ysc); viAreas.add(courtT);
  ...
  courtB = new Panel(250*Xsc,170*Ysc,350*Xsc,320*Ysc); viAreas.add(courtB);
  ...
  //CR, L,C
  //others = new Panel(220*Xsc,10*Ysc,350*Xsc,180*Ysc)
  default_text_corner_radius=5;
  tmp=new TextButton("CR",0*Xsc,0*Ysc,20*Xsc,20*Ysc);buttons.add(tmp);others.add(tmp); 
  tmp=new TextButton("L",0*Xsc,30*Ysc,20*Xsc,50*Ysc);buttons.add(tmp);others.add(tmp); 
  tmp=new TextButton("C",0*Xsc,60*Ysc,20*Xsc,80*Ysc);buttons.add(tmp);others.add(tmp); 
